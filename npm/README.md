...
`npm install markdownlint-rs` bundles all 7 platform binaries in the package.
The `mdlint` command is a thin Node.js wrapper that selects and execs the correct binary for your OS and
architecture, using `/proc/self/maps` to distinguish glibc vs musl on Linux.
...
repos:
  - repo: https://github.com/swanysimon/mdlint
    rev: v0.3.17  # use the latest release tag
    hooks:
...
Releases are automated via `.github/workflows/publish-npm.yml`. On a version tag push, the workflow downloads each
pre-built binary from the GitHub release and publishes the package to npm via trusted publishing (OIDC, no token
required).
